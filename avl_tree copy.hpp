...
public:
    ...
    std::vector<pair<T, size_t>> to_vectormap() const;
    MultiSet<T> copy() const;
    ...
    MultiSet<T> symmetric_difference(const MultiSet<T> &other) const;
    MultiSet<T> &operator+=(const MultiSet<T> &other);
    MultiSet<T> &operator|=(const MultiSet<T> &other);
    MultiSet<T> &operator&=(const MultiSet<T> &other);
    MultiSet<T> &operator-=(const MultiSet<T> &other);
    MultiSet<T> &operator^=(const MultiSet<T> &other);
    bool is_subset(const MultiSet<T> &other) const;
    bool is_propersubset(const MultiSet<T> &other) const;
    bool is_equal(const MultiSet<T> &other) const;
...
template <typename T>
void MultiSet<T>::insert(const T &key, size_t amount = 1)
{
    root = insert(root, key, amount);
}

template <typename T>
void MultiSet<T>::remove(const T &key, size_t amount = 1)
{
    if (amount <= 0)
        return;
